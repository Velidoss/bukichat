...
const messagesSlice = createSlice({
  ...
  reducers: {
    ...
    addLike: (state, action) => {
      ...
      const likeIndex = checkIfLikeOrDislikeExists(
        current(state).messages[messageIndex].likes, newLike,
      );
      if (likeIndex >= 0) {
    ...
    removeLike: (state, action) => {
      ...
      const likeIndex = checkIfLikeOrDislikeExists(
        current(state).messages[messageIndex].likes, newDisLike,
      );
      if (likeIndex >= 0) {
        if (state.messages[messageIndex].likes[likeIndex].value < 0) {
          state.messages[messageIndex].likes = state
...
export const {
  addNewMessage, addLike, removeLike, editMessageAC, deleteMessageAC,
